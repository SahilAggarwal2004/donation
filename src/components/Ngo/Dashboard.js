...
const Dashboard = () => {
  ...
  useEffect(() => {
    ...
    getAllDonations(data?.pincode).then(({ success, donations }) => { if (success) setDonations(donations) })
  }, [])
          ...
          {donations?.map(({ uid, name, image, mobile, address, id, description, status, otp }) => <div className="shadow-[0_1px_16px_2px_rgba(0,0,0,.2)] rounded-xl  px-2 sm:px-8 py-6 flex  flex-col gap-6 bg-slate-50 hover:bg-gray-100 hover:text-[#2dc1e4] hover:border-[#2dc1e4] hover:border-2 transition-all duration-200 ease-in-out ">
            ...
            <div className="flex flex-col gap-2 sm:flex-row sm:gap-0 justify-between items-center ">
              <div className="flex flex-col sm:flex-row  gap-4 items-center">
                <button onClick={() => accept(uid, id)} className="bg-green-400 text-white rounded-2xl px-6 py-2 text-xl font-medium  hover:bg-gray-100 hover:text-green-400 hover:border-green-400 hover:border-2 transition-all duration-200 ease-in-out ">
                  Accept
                </button>
